...
-- Existing DBs: apply share_id by hand (CREATE TABLE above no-ops on an existing
-- table). In the Neon console, before creating the index:
--   ALTER TABLE runs ADD COLUMN IF NOT EXISTS share_id TEXT UNIQUE NOT NULL
--     DEFAULT substr(replace(gen_random_uuid()::text, '-', ''), 1, 12);
